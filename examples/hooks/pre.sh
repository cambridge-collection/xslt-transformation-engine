...
usage() {
  cat >&2 <<'USAGE'
Usage: pre.sh --source-dir <dir> [ --includes-file <path> | --pattern <glob> ] --out-dir <dir>
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)
      ...
      # If the token is an existing file, assume includes-file; else treat as pattern
      if [[ -z "$VALUE" ]]; then
...
copy_matches() {
  ...
  (
    ...
    log_info "pre.hook copied $count file(s)"
  )
